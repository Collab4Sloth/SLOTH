
/**
 * @file HDF54Sloth.cpp
 * @author cp273896  (clement.plumecocq@cea.fr)
 * @brief Class to define interaction between hdf5 files and sloth
 * @version 0.1
 * @date 2025-03-19
 *
 * Copyright CEA (c) 2025
 *
 */

#include <functional>
#include <memory>
#include <string>
#include <tuple>
#include <utility>
#include <vector>

#include "H5Cpp.h"
#include "Utils/Utils.hpp"
#include "Utils/UtilsForTensorialAlgebra.hpp"
#pragma once

template <typename T>
class HDF54Sloth {
 public:
  HDF54Sloth();
  static void get_data_from_HDF5(const H5std_string& file_name, const std::string& dataset_name,
                                 std::vector<double>& output_multi_array);
  static void get_data_from_HDF5(const H5std_string& file_name, const std::string& dataset_name,
                                 FlattenedTensor<double>& output_multi_array);
  static void write_data_to_HDF5(const H5std_string& file_name, const std::string& dataset_name,
                                 const std::vector<double>& input_multi_array);

  static std::vector<H5::Group> explore_HDF5file(const H5::Group& group,
                                                 const std::string& path = "/");

  ~HDF54Sloth();
};
template <typename T>
HDF54Sloth<T>::HDF54Sloth() {}

template <typename T>
std::vector<H5::Group> HDF54Sloth<T>::explore_HDF5file(const H5::Group& group,
                                                       const std::string& path) {
  std::vector<H5::Group> list_sub_group = {};
  hsize_t numObjs = group.getNumObjs();
  for (hsize_t j = 0; j < numObjs; ++j) {
    std::string name = group.getObjnameByIdx(j);
    H5G_obj_t type = group.getObjTypeByIdx(j);
    if (type == H5G_GROUP) {
      H5::Group sub_group = group.openGroup(name);
      list_sub_group.push_back(sub_group);
    }
  }
  return list_sub_group;
}

/**
 * @brief Get a vector store in HDF5 dataset
 *
 * @tparam T
 * @param file_name
 * @param dataset_name
 * @param output_multi_array
 */
template <typename T>
void HDF54Sloth<T>::get_data_from_HDF5(const H5std_string& file_name,
                                       const std::string& dataset_name,
                                       std::vector<double>& output_multi_array) {
  bool is_hdf5 = H5::H5File::isHdf5(file_name);
  H5::H5File file(file_name, H5F_ACC_RDONLY);
  H5::DataSet dataset = file.openDataSet(dataset_name);
  H5::DataSpace dataspace = dataset.getSpace();
  int rank = dataspace.getSimpleExtentNdims();
  std::vector<hsize_t> dims(rank);
  dataspace.getSimpleExtentDims(dims.data(), NULL);
  size_t total_size = 1;
  for (size_t dim : dims) {
    total_size *= dim;
  }
  output_multi_array.resize(total_size);
  dataset.read(output_multi_array.data(), H5::PredType::NATIVE_DOUBLE);
}

/**
 * @brief Get a vector store in HDF5 dataset
 *
 * @tparam T
 * @param file_name
 * @param dataset_name
 * @param output_multi_array
 */
template <typename T>
void HDF54Sloth<T>::get_data_from_HDF5(const H5std_string& file_name,
                                       const std::string& dataset_name,
                                       FlattenedTensor<double>& output_multi_array) {
  bool is_hdf5 = H5::H5File::isHdf5(file_name);
  H5::H5File file(file_name, H5F_ACC_RDONLY);
  H5::DataSet dataset = file.openDataSet(dataset_name);
  H5::DataSpace dataspace = dataset.getSpace();
  std::size_t rank = dataspace.getSimpleExtentNdims();
  std::vector<hsize_t> dims(rank);
  output_multi_array.set_dim(rank);

  dataspace.getSimpleExtentDims(dims.data(), NULL);
  std::vector<size_t> size_t_dims(dims.begin(), dims.end());
  output_multi_array.set_shape(size_t_dims);
  size_t total_size = 1;
  for (size_t dim : size_t_dims) {
    total_size *= dim;
  }
  output_multi_array.resize(total_size);
  dataset.read(output_multi_array.data(), H5::PredType::NATIVE_DOUBLE);
}

template <typename T>
void HDF54Sloth<T>::write_data_to_HDF5(const H5std_string& file_name,
                                       const std::string& dataset_name,
                                       const std::vector<double>& input_multi_array) {
  std::ifstream file_check(file_name.c_str());
  bool file_exists = std::filesystem::exists(file_name);

  H5::H5File file(file_name, file_exists ? H5F_ACC_RDWR : H5F_ACC_TRUNC);
  std::vector<hsize_t> dims(1);
  dims[0] = input_multi_array.size();
  H5::DataSpace dataspace(1, dims.data());
  H5::DataSet dataset = file.createDataSet(dataset_name, H5::PredType::NATIVE_DOUBLE, dataspace);
  dataset.write(input_multi_array.data(), H5::PredType::NATIVE_DOUBLE);
}

template <typename T>
HDF54Sloth<T>::~HDF54Sloth() {}
