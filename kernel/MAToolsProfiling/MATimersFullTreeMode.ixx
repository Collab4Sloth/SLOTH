...
#include <MAToolsProfiling/MADebug.hxx>
#include <MAToolsProfiling/MAOutputManager.hxx>
#include <MAToolsProfiling/MATimerMPI.hxx>
...
/**
 * build a vector of minimal info per MPI process to send them (after this routine) on the master
 * process.
 * @see class minimal_info
...
/**
 * build a vector of minimal info on the master process. Every trees are packed in a vector of
 * minimal_info and they are gathered on the master node.
 * @see class minimal_info
 * @return void
 */
void build_full_tree() {
  ...
  MPI_Allgatherv(my_info.data(), info_size, MPI_CHAR, recv.data(), sizes.data(), displ.data(),
