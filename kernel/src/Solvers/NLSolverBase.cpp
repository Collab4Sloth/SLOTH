...
/**
 * @brief Construct a new SolverNewton object
 *
 */
 ...
 */
std::shared_ptr<mfem::NewtonSolver> SolverNewton::create_solver(const Parameters& params) {
  this->solver_description_ = params.get_param_value<std::string>("description");
  int print_level = NewtonDefaultConstant::print_level;
...
/**
 * @brief Destroy the SolverNewton object
 *
 */
SolverNewton::~SolverNewton() {}

/**
 * @brief Construct a new SolverLBFGS object
 *
 */
 ...
 *
 * @param params
 * @return std::shared_ptr<mfem::LBFGSSolver>
 */
std::shared_ptr<mfem::LBFGSSolver> SolverLBFGS::create_solver(const Parameters& params) {
  this->solver_description_ = params.get_param_value<std::string>("description");
  int print_level = LBFGSDefaultConstant::print_level;
  if (verbose_at_least(Verbosity::Verbose)) {
    print_level =
        params.get_param_value_or_default<int>("print_level", LBFGSDefaultConstant::print_level);
  }
  const bool iterative_mode = params.get_param_value_or_default<bool>(
      "iterative_mode", LBFGSDefaultConstant::iterative_mode);
  const int history_size =
      params.get_param_value_or_default<int>("history_size", LBFGSDefaultConstant::history_size);
  const int n_iter_max =
      params.get_param_value_or_default<int>("iter_max", NewtonDefaultConstant::iter_max);
  const double n_rel_tol =
      params.get_param_value_or_default<double>("rel_tol", NewtonDefaultConstant::rel_tol);
  ...
  auto ss = std::make_shared<mfem::LBFGSSolver>(MPI_COMM_WORLD);

  ss->SetPrintLevel(print_level);
  ss->SetMaxIter(n_iter_max);
  ss->iterative_mode = iterative_mode;
  ss->SetHistorySize(history_size);
  ss->SetRelTol(n_rel_tol);
  ss->SetAbsTol(n_abs_tol);
...
/**
 * @brief Destroy the SolverLBFGS object
 *
 */
SolverLBFGS::~SolverLBFGS() {}
