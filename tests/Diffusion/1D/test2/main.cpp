...
int main(int argc, char* argv[]) {
  ...
  //---------------------------------------
  const auto DIM = 1;
  using NLFI = DiffusionNLFormIntegrator<CoefficientDiscretization::Explicit, Diffusion::Constant>;
  ...
  double L = 1e-3;
  // std::vector<int> vect_NN{10,20,40,80,160,320};
    std::vector<int> vect_NN{10,20,40,80,160,320};
  ...
  auto vars = VAR(Variable<FECollection, DIM>(&spatial, bcs, "c", 2, initial_condition,analytical_solution));
  ...
  const auto& t_final = 3.;  // 0.5;
  const auto& dt = 0.00001;//0.5 * ((L/NN)*(L/NN))/ (2 * diffusionCoeff);
