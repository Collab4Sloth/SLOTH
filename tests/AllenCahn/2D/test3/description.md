# 2D Allen-Cahn simulation in a square : steady linear manufactured solution

## Statement of the problem
This example code solves the Allen-Cahn equation in a 2D domain $\Omega=[0,1]\times[0,1]$ (inline mesh), with a spatial source term built in order to recover a steady linear solution:

$$
u({\bf x})=2x
$$

### __Governing equation__
Let us consider the following set of governing equations:

$$
a\Delta u - H \partial_uf_{dw}(u)+S({\bf x}) = 0 \text{ in }\Omega 
$$

$$
f_{dw}(u)=u^2(1-u)^2
$$

where $S({\bf x})=2 u({\bf x})  (1 - 2u({\bf x})) (1 - u({\bf x}))$ 



### __Boundary conditions__

Neumann boundary conditions are prescribed on $\Gamma_{lower}$ (y=0) and $\Gamma_{upper}$ (y=R):
$$
{\bf{n}} \cdot{} (a\nabla u) =0 \text{ on }\Gamma_{lower}  \cup \Gamma_{upper}
$$


Dirichlet boundary conditions are prescribed on $\Gamma_{left}$ :

$$
u=0 \text{ on }\Gamma _{left}
$$

Dirichlet boundary conditions are prescribed on $\Gamma_{right}$ :

$$
u=2 \text{ on }\Gamma _{right}
$$

### __Parameters__
For this test, all parameters are set to one.

### __Numerical scheme__

- Newton solver
- Spatial discretization: uniform mesh with $N=4$ nodes in each direction


  
## Running

### __Using the binary__
```shell
./AllenCahn2Dtest3
```

### __Using ctest__

```shell
ctest -R AllenCahn2Dtest3
```


## Post-processing

(to be written)

## Files & Dependencies

Source file : `main.cpp`

## References

None

## Intellectual Property

See [About page](../../../../../about.html) 