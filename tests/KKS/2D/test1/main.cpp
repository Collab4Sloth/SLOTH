...
int main(int argc, char* argv[]) {
  ...
  // KKS
  using PB_KKS = InterfaceProblem<VAR, PST>;
  ...
  const auto& radius = 5e-4;

  auto ac_ic = AnalyticalFunctions<DIM>(AnalyticalFunctionsType::HyperbolicTangent, center_x, a_x,  thickness, radius);
  auto ac_vars = VAR(Variable<FECollection, DIM>(&spatial, bcs, "phi", 2, ac_ic));

  // Heat
  auto heat_vars = VAR(Variable<FECollection, DIM>(&spatial, Tbcs, "T", 2, ac_ic));
  auto pl = 15.e4;
  auto src_func = std::function<double(const mfem::Vector&, double)>(
      [pl, pellet_radius](const mfem::Vector& vcoord, double time) {
        ...
        const auto func =  0.;//pl * bess / (M_PI * 2. * pellet_radius * pellet_radius);
  ...
  // KKS
  PB_KKS KKS_pb("KKS", ac_vars, pst3, convergence, ac_vars, heat_vars);


  // Coupling 1
  auto cc = Coupling("AC-Heat coupling", allencahn_pb, Heat_pb, KKS_pb);
